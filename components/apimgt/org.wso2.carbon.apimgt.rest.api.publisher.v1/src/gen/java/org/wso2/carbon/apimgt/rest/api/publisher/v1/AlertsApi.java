...
public class AlertsApi  {
    ...
    @ApiOperation(value = "Add AbnormalRequestsPerMin Alert Configurations. ", notes = "This operation is used to add configuration for the AbnormalRequestsPerMin alert type. ", response = AlertConfigDTO.class, authorizations = {
        @Authorization(value = "OAuth2Security", scopes = {
            @AuthorizationScope(scope = "", description = "")        })    }, tags={ "Alert Configuration" })
    @ApiResponses(value = { 
        @ApiResponse(code = 201,
                    message = "Created. Successful response with newly created object as entity. Location header contains URL of newly created entity. ",
                    response = AlertConfigDTO.class),
        ...
        @ApiResponse(code = 500,
                    ...
                    response = ErrorDTO.class) })
    public Response addAlertConfig(

        @ApiParam(value = "Configuration for AbnormalRequestCount alert type",required=true) Map<String, String> body
, 

        @ApiParam(value = "The alert type.",required=true)
            @PathParam("alertType") String alertType
, 

        @ApiParam(value = "The alert configuration id.",required=true)
            @PathParam("configurationId") String configurationId
) throws APIManagementException{
        return delegate.addAlertConfig(body, alertType, configurationId, securityContext);
    }
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200,
                    message = "OK. The alert config is deleted successfully. ",
                    response = Void.class),
        @ApiResponse(code = 400,
                    message = "Bad Request. Invalid request or validation error.",
                    response = ErrorDTO.class),
...
, 
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200,
                    message = "OK. The store alert configuration. ",
                    response = AlertConfigListDTO.class),
        @ApiResponse(code = 500,
                    message = "Internal Server Error.",
                    response = ErrorDTO.class) })
