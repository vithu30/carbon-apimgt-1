...
public class AlertSubscriptionsApi  {
    ...
    @ApiOperation(value = "Get the List of API Publisher Alert Types Subscribed by the User ", notes = "This operation is used to get the list of subscribed alert types by the user. ", response = AlertsInfoDTO.class, authorizations = {
        @Authorization(value = "OAuth2Security", scopes = {
            @AuthorizationScope(scope = "", description = "")        })    }, tags={ "Alert Subscriptions" })
    @ApiResponses(value = { 
        @ApiResponse(code = 200,
                    message = "OK. The list of subscribed alert types are returned. ",
                    response = AlertsInfoDTO.class),
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 201,
                    message = "OK. Successful response with the newly subscribed alerts. ",
                    response = AlertsInfoResponseDTO.class),
        ...
        @ApiResponse(code = 500,
                    message = "Internal Server Error.",
                    response = ErrorDTO.class) })
    public Response subscribeToAlerts(

        @ApiParam(value = "The alerts list and the email list to subscribe.",required=true) AlertsInfoDTO body
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200,
                    message = "OK. The user is unsubscribed from the alerts successfully. ",
                    response = Void.class),
        @ApiResponse(code = 500,
                    message = "Internal Server Error.",
                    response = ErrorDTO.class) })
