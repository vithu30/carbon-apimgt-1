...
public class SubscriptionsApi  {
    ...
    @ApiOperation(value = "Block a Subscription", notes = "This operation can be used to block a subscription. Along with the request, `blockState` must be specified as a query parameter.  1. `BLOCKED` : Subscription is completely blocked for both Production and Sandbox environments. 2. `PROD_ONLY_BLOCKED` : Subscription is blocked for Production environment only. ", response = Void.class, authorizations = {
        @Authorization(value = "OAuth2Security", scopes = {
            @AuthorizationScope(scope = "", description = "")        })    }, tags={ "Subscriptions" })
    @ApiResponses(value = { 
        @ApiResponse(code = 200,
                    message = "OK. Subscription was blocked successfully. ",
                    response = Void.class),
        ...
        @ApiResponse(code = 412,
                    ...
                    response = ErrorDTO.class) })
    public Response subscriptionsBlockSubscriptionPost( @NotNull 
        @ApiParam(value = "Subscription Id ",required=true) 
            @QueryParam("subscriptionId") String subscriptionId
,  @NotNull 
        @ApiParam(value = "Subscription block state. ",required=true, allowableValues="BLOCKED, PROD_ONLY_BLOCKED") 
...
, 
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200,
                    message = "OK. Subscription list returned. ",
    ...
    public Response subscriptionsGet(
        @ApiParam(value = "**API ID** consisting of the **UUID** of the API. The combination of the provider of the API, name of the API and the version is also accepted as a valid API I. Should be formatted as **provider-name-version**. ") 
...
, 
        @ApiParam(value = "Keywords to filter subscriptions ") 
    ...
    @ApiOperation(value = "Get Details of a Subscriber", notes = "This operation can be used to get details of a user who subscribed to the API. ", response = SubscriberInfoDTO.class, authorizations = {
        @Authorization(value = "OAuth2Security", scopes = {
            @AuthorizationScope(scope = "", description = "")        })    }, tags={ "Subscriber" })
    @ApiResponses(value = { 
        @ApiResponse(code = 200,
                    message = "OK.  Details of the subscriber are returned. ",
                    response = SubscriberInfoDTO.class),
        @ApiResponse(code = 404,
                    message = "Not Found. The specified resource does not exist.",
                    response = ErrorDTO.class) })
    public Response subscriptionsSubscriptionIdSubscriberInfoGet(
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200,
                    message = "OK. Details of a pending invoice returned. ",
                    response = APIMonetizationUsageDTO.class),
        @ApiResponse(code = 304,
                    message = "Not Modified. Empty body because the client has already the latest version of the requested resource (Will be supported in future). ",
                    response = Void.class),
        @ApiResponse(code = 404,
                    message = "Not Found. Requested Subscription does not exist. ",
                    response = ErrorDTO.class) })
    public Response subscriptionsSubscriptionIdUsageGet(
    ...
    @ApiResponses(value = { 
        @ApiResponse(code = 200,
                    message = "OK. Subscription was unblocked successfully. ",
                    response = Void.class),
        @ApiResponse(code = 400,
                    message = "Bad Request. Invalid request or validation error.",
                    response = ErrorDTO.class),
        ...
        @ApiResponse(code = 412,
                    message = "Precondition Failed. The request has not been performed because one of the preconditions is not met.",
                    response = ErrorDTO.class) })
    public Response subscriptionsUnblockSubscriptionPost( @NotNull 
        @ApiParam(value = "Subscription Id ",required=true) 
            @QueryParam("subscriptionId") String subscriptionId
, 
        @ApiParam(value = "Validator for conditional requests; based on ETag. " )
